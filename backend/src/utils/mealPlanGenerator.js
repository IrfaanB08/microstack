...
const { sumMealMacros, scoreMacroFit } = require('./macroScoring');
...
class MealPlanGenerator {
   ...
   * Shared day-so-far tally used by assignMealsToSlots/selectRecipeForSlot
   * and by calculateDailyMacros - see macroScoring.js.
   */
  sumMealMacros(meals) {
    return sumMealMacros(meals);
  }
  ...
  averageMacroTargets(macroTargetsByDay) {
    const sum = sumMealMacros(this.daysOfWeek.map((day) => macroTargetsByDay[day]));
  ...
  /**
   * Score how well a candidate's macros fit a target across all four
   * macros, not calories alone - see macroScoring.js.
   */
  scoreMacroFit(candidateMacros, targetMacros) {
    return scoreMacroFit(candidateMacros, targetMacros);
  }
